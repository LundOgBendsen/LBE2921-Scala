...
object CaseClassesPatternMatching {
  ...
  def petInfo(pet: Pet): String = pet match {
    case Snake(n, toxic, _, _) if (toxic == true) => s"DANGER! TOXIC SNAKE, dont touch!!! ($n)"
  ...
  myPets.map(petInfo).foreach(println)            //> Simba
                                                  //| Rex
                                                  //| DANGER! TOXIC SNAKE, dont touch!!! (misssssing)
                                                  //| %n,  a harmless snake
                                                  //|  <3<3<3 Dark Queen ... All bills shared in 1/2
}
