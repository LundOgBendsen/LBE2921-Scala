...
object PolymorphismFun {
   val twoParams = List(1, 2)                     //> twoParams  : dk.lundogbendsen.scala.labs.polymorphism_07.List[Int] = 1{2{.}}
                                                  //| 
...
// Discuss - when do we use a trait, and when do we use an abstract class? ... we shadow the actual list here
trait List[T] {
    def isEmpty:Boolean
    def head: T
    def tail: List[T]
