...
object objects {
  val x = new Rational(1,3)                       //> x  : objects_and_data.Rational = 1/3
  //x.numerator
 // x.denominator
  
  val z = new Rational(1,2)                       //> z  : objects_and_data.Rational = 1/2
  
  val y = x.add(z)                                //> y  : objects_and_data.Rational = 5/6
  
  val negy = y.negative                           //> negy  : objects_and_data.Rational = -5/6
  
  val res = negy add(z) add(x)                    //> res  : objects_and_data.Rational = 0/1
  
  y.substract(z)                                  //> res0: objects_and_data.Rational = 1/3
  
  x < z                                           //> res1: Boolean = true
  
  x.max(z)                                        //> res2: objects_and_data.Rational = 1/2
  
  //uncomment to see precondition error!
  //val strange = new Rational(1,0)
 // strange.add(strange)
 
 val integer = new Rational(1)                    //> integer  : objects_and_data.Rational = 1/1
